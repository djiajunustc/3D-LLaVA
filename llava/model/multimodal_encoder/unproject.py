...
def voxelize(xyz, offset, voxel_size=0.28):
    """
    Inputs: 
    ...
    for i in range(batch_size):
        cur_xyz = xyz[count:offset[i]]
        count = offset[i]
        cur_xyz = cur_xyz.reshape(1, -1, 3)
        cur_p2v = voxelization(cur_xyz, voxel_size)
        p2v_list.append(cur_p2v.squeeze(0))
